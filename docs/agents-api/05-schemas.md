...
| `retrieval_mode`   | `string \| null`           | `full` or `matching`. Only set for a `document` task — whether the whole attachment was needed or one point out of it. |

`agent: "document"` means the search read the documents attached to the turn rather than a corpus.
